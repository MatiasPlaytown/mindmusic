...
- **Temas sueltos**: cada canción tiene su propio arte, generado a partir del **mood + el nombre del tema**. `songArtVars(mood, title, photo)` (app.js) hashea `moodId + título` y de ahí salen el recorte de la foto, el ángulo del degradé y la posición de las capas de color; dentro de una playlist la foto base es la de esa playlist (un tema suelto usa la del mood); las **iniciales del tema** (`songInitials`) firman la portada. Ese arte se dibuja como cuadrado (`.songart`, `songArtHTML`) en las filas de tracklist, en la cola del player y en grande apoyado sobre la banda de ondas (`.player-art`, `.songart--lg`), y como fondo de la funda en las tarjetas de grilla (`.cover--art` + `.cover-initials`). Determinístico: la misma canción se ve siempre igual.
